...
## 2. 데이터 분석
### 2.1 데이터 수집 방법 (웹 크롤링)
- **리뷰 사이트 접근** : 구글 플레이 스토어의 로블록스 페이지에서 리뷰 데이터를 포함하는 각 리뷰의 텍스트 및 평점 정보를 추출하였습니다.
...
## 4. 느낀점 및 배운점
로블록스 리뷰 데이터를 분석하며 대부분의 유저들이 로블록스를 긍정적으로 평가하고 있다는 사실을 알 수 있었다. 본인 또한 유저로서 로블록스를 즐기고 있으며, 분석 결과를 통해 다른 유저들의 생각과 공감할 수 있었다. 하지만 이번 프로잭트의 결과를 내지 못하여 아쉬운 점이 많아 더 노력해야겠다고 생각한다.
